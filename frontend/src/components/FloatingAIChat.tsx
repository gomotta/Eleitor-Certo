...
interface Props {
  ...
  rightOffset?: number;
  /** Margem esquerda da barra (px). Se informado, a barra deixa de ser centralizada e passa a respeitar left/right. */
  leftInset?: number;
  /** Margem direita da barra (px). Se informado, a barra deixa de ser centralizada e passa a respeitar left/right. */
  rightInset?: number;
  /** Se true, mantém a barra sempre aberta */
  forceOpen?: boolean;
  /** Se true, esconde o botão flutuante de abrir/fechar */
...
export default function FloatingAIChat({
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-6 z-[2000] transition-all duration-300 ${
          forceOpen || open
            ? 'opacity-100 translate-y-0 pointer-events-auto'
            : 'opacity-0 translate-y-4 pointer-events-none'
        }`}
        style={{
          ...(leftInset != null && rightInset != null
            ? {
                left: `${leftInset}px`,
                right: `${rightInset}px`,
                width: 'auto',
      ...
      >
        <div className="mx-auto" style={{ width: 'min(760px, 100%)' }}>
